...
public class PcCafeMain extends JFrame implements ActionListener, ListSelectionListener, ItemListener, MouseListener{
	...
	public void actionPerformed(ActionEvent e){
		...
		if(str.equals("시간 추가")) {
			...
			try {
				Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/odbo?serverTimezone=UTC","root","1380");
...
class LoginMain extends JFrame implements ActionListener{
	...
	LoginMain(){
		...
		JLabel log = new JLabel("PC CAFE 포스기");
			...
			if(s.equals("조회")) {
				...
				if(CEO == 1) {
					if(click == 1) {
						...
						strSql = "INSERT INTO work_info(wor	k_num,id,year,month,day,start_time,finish_time) values('"+WorkTable.getValueAt(0,0)+"',(select id from worker_info where name='"+WorkTable.getValueAt(0,1)+"'),'"+WorkTable.getValueAt(0,2)+"','"+WorkTable.getValueAt(0,3)+"','"+WorkTable.getValueAt(0,4)+"','"+WorkTable.getValueAt(0,5)+"','"+WorkTable.getValueAt(0,6)+"');";
						dbSt.executeUpdate(strSql);
					}
				}
			}
...
class UseDB 
{
	Statement dbSt;
	Connection con;
	void updateDB(String strSql) throws SQLException,ClassNotFoundException 
	{
		Class.forName("com.mysql.jdbc.Driver");  
		System.err.println("MYSQL JDBC 드라이버를 정상적으로 로드함");
		con=DriverManager.getConnection("jdbc:mysql://localhost:3306/odbo?serverTimezone=UTC","root","1380");
		System.out.println("DB 연결 완료."); 
		dbSt = con.createStatement();
		System.out.println("JDBC 드라이버가 정상적으로 연결되었습니다.");
		dbSt.executeUpdate(strSql); 
	}
	ResultSet queryDB(String strSql) throws SQLException,ClassNotFoundException 
	{
		Class.forName("com.mysql.jdbc.Driver");  
		System.err.println("MYSQL JDBC 드라이버를 정상적으로 로드함");
		con=DriverManager.getConnection("jdbc:mysql://localhost:3306/odbo?serverTimezone=UTC","root","1380");
...
class Overlap extends JDialog implements ActionListener 
{
	JButton ok;
...
/* --------------------- 기본 ------------------------------- */
/* ----------------------메뉴 수정 시작------------------------------- */
class MenuEditForm extends JFrame implements ListSelectionListener, ActionListener 
{
	JList jlst_menu;
	...
	void reNewSection() throws SQLException,ClassNotFoundException 
	{
		UseDB db = new UseDB();
	...
	void reNewMenu() throws SQLException,ClassNotFoundException
	{
		UseDB db = new UseDB();
		model.setNumRows(0); 
	...
	public void valueChanged(ListSelectionEvent lse)  
	{
		...
		{
			select = jlst_menu.getSelectedValue().toString(); 
			lselect = jlst_menu.getSelectedIndex(); 
			try
			{
				reNewMenu();
			}
...
class MenuEdit extends JFrame implements ActionListener 
{
	...
	JLabel[] jlb_values;
	JTextField[] jtf_edit;
	MenuEdit(String title, String section, int row, TableModel table){ 
		ct = getContentPane();
	...
	void update() 
	{
		...
		try
		{
			String strSql = "update menu set section='" + values[0] + "', name='" + values[1] + "', price=" 
				+ Integer.parseInt(values[2])+ ", stock=" + Integer.parseInt(values[3]) + " where name='"+select+"';"; 
		db.updateDB(strSql);
		db.closeDB();
		dispose();
		}
	...
	void insert() throws SQLException, NumberFormatException,ClassNotFoundException 
	{
		UseDB db = new UseDB();
		for (int i = 0; i < values.length ; i++ )
		{
		...
		strSql = "insert ignore into menu values ('" + values[0] +"', '" + values[1] + "', "+ values[2] +", "+ values[3] +");"; 
		db.updateDB(strSql); 
		db.closeDB();
		dispose();
	}
	void delete() throws SQLException,ClassNotFoundException 
	{
		...
		strSql = "delete from menu where name='" + tuple[1] + "';"; 
		db.updateDB(strSql); 
		db.closeDB();
		dispose();
	}
...
class SectionEdit extends JFrame implements ActionListener 
{
	Container ct;
	String title;
	JLabel jlb_select;
	...
	void insert() throws SQLException,ClassNotFoundException 
	{
		...
		strSql = "insert ignore into menu_section values ('" + row +"', '" + edit + "');"; 
		db.updateDB(strSql); 
		db.closeDB();
		dispose();
	}
...
/* ----------------------메뉴 수정 끝--------------------------------- */
/* ------------------------------- 주문 시작 ------------------------------- */
class OrderForm extends JFrame implements ListSelectionListener, ActionListener 
{
	...
	void payments() throws ClassNotFoundException, SQLException 
	{
		UseDB db = new UseDB();
		payment = 0;
	...
	void sectionSelect(String select) throws ClassNotFoundException, SQLException 
	{
		UseDB db = new UseDB();
	...
	{
		boolean flag = true;
		int row[] = jtab_menuSelect.getSelectedRows(); 
	...
	class SearchMenu extends JFrame implements ActionListener 
	{
		String goods;
		...
		SearchMenu(String item)
		{
			...
			jtab_menuSearch.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
		...
		void search(String goods) throws ClassNotFoundException, SQLException 
		{
			UseDB db = new UseDB();
			model.setRowCount(0);
		...
		void searchSelect() throws ClassNotFoundException, SQLException 
		{
			int sectionIndex = -1;
...
/* ------------------------- 주문 끝 --------------------------------------------- */
/* ------------------------- 시간 추가 시작 --------------------------------------------- */
class TimeOrderForm extends JFrame implements ActionListener 
{
	...
	{
		hour = 0; minute = 0; id = user_id; flag=true;
	...
	public void actionPerformed(ActionEvent ae) 
	{
		...
		else if (ae.getActionCommand().replace(" ","").contains("결제")) 
		{
			UseDB db = new UseDB();
			try
			{
...
/* ------------------------- 시간 추가 끝 --------------------------------------------- */
/* ------------------------- 환불 조회 시작 --------------------------------------------- */
class RefundInqueryForm extends JFrame implements ActionListener 
{
	...
	public void actionPerformed(ActionEvent ae) 
	{
		try
		{
			...
			else if (ae.getActionCommand().toString().contains("상세"))
			{
				...
				if (row == -1)
					rif = new RefundDetailInqueryForm(); 
				else
					rif = new RefundDetailInqueryForm(model.getValueAt(jtab_list.getSelectedRow(),0)+""); 
				rif.show();
			}
...
class RefundDetailInqueryForm extends JFrame implements ActionListener 
{
	JTextField jtf_num;
	JButton jbtn_search;
	JLabel jlb_Sales;
	...
	String number;
	RefundDetailInqueryForm(String num) 
	{
		number = num;
		constructor();
		
	}
	RefundDetailInqueryForm() 
	{
		number = "";
		constructor();
...
/* ------------------------- 환불 조회 끝 --------------------------------------------- */
/* ------------------------- 거래 조회 시작 --------------------------------------------- */
class TradeInqueryForm extends JFrame implements ActionListener 
{
	JTextField jtf_id, jtf_num;
	JLabel jlb_todaySales;
...
class TradeDetailInqueryForm extends JFrame implements ActionListener 
{
	JTextField jtf_num;
	JButton jbtn_search;
	JLabel jlb_Sales;
...
class DateInquery extends JPanel implements ActionListener, ItemListener 
{
	JComboBox jcb_start[], jcb_end[];
...
class MenuInquery extends JPanel implements ActionListener, ItemListener, ListSelectionListener 
{
	JComboBox jcb_start[], jcb_end[];
	...
	void search() 
	{
		...
		try
		{
			UseDB db = new UseDB();
			String strSql = "";
			if (rbChk == 0 && jlst_section.getSelectedIndices().length > 0) 
			{
			...
			else if (rbChk == 1 && jlst_menu.getSelectedIndices().length > 0) 
			{
			...
			else if (rbChk == 2 && jlst_menu.getSelectedIndices().length > 0 && jlst_section.getSelectedIndices().length > 0) 
			{
	...
	{	
		...
		rbChk = -1;
		if ( jchk_section[0].isSelected() ) 
			rbChk = 0; 
		if ( jchk_section[1].isSelected() )  
			rbChk = 1; 
		if ( jchk_section[0].isSelected() && jchk_section[1].isSelected() )  
			rbChk = 2;
	}
	public void valueChanged(ListSelectionEvent lse) 
	{
...
class UserInquery extends JPanel implements ActionListener, ItemListener 
{
	JComboBox jcb_start[], jcb_end[];
	JButton jbtn_search;
	JTable jtab_list;
